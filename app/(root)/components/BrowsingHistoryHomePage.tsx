...
const BrowsingHistoryHomePage = () => {
  ...
  return (
    <div className="border-[1px] rounded-md p-4 bg-white my-5">
      <h1 className="text-xl font-medium my-4 text-slate-800">From Your Browsing History</h1>
      <div className="w-full overflow-x-auto" style={{ overflowX: 'auto', scrollbarWidth:"none" }}>
        <div className="flex items-start flex-row gap-4 ">
          {BrowsingHistoryProducts.map((product: any) => {
            ...
            return (
              <TopDealsProductsCard product={product} key={product.id} isInCart={!!isProductInCart} />
            )
          })}
        </div>
      </div>
